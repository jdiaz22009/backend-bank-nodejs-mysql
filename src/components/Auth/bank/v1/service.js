...
services.updateClient = data => new Promise((resolve, reject) => {
    let query = `SELECT * FROM clients WHERE email = '${data.email}'`
    data.create_update = currentDate;
    sql.select(query)
        .then(res => {
            if (res.code == 200) {
                ...
                for (let users of user) {
                    let query = `UPDATE clients SET name = '${data.name}', last_name = '${data.last_name}', document = ${data.document}, email= '${data.email}' , phone_mobile = ${data.phone_mobile}, age = ${data.age}, create_update = '${data.create_update}' WHERE email = '${users.email}'`
                    sql.update(query)
                        .then(res => {
                            return resolve(res)
                        })
